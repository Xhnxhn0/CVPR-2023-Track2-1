...
def main():
    ...
    for vehicle_image_name_fn in tqdm(vehicle_image_names_list):
        try:
            ...
            error_img_names.append(vehicle_image_name_fn)
            continue
        ...
        # else:
        #     save_file = op.join(args.save_file, "img", person_image_name)
        # save_file = op.join(args.save_file, "img", person_image_name)
        ...
        if cfg.MODEL.ATTRIBUTE_ON and args.visualize_attr:
            ...
            file_name = os.path.basename(vehicle_image_name_fn).split(".")[0] + ".txt"
            print("file_name is ", file_name)
            text_save_file = op.join(save_dir, file_name)
            with open(text_save_file, "w") as fid:
                fid.write(result_str)

    print("error error_img_names len is ", len(error_img_names))
    with open(os.path.join(args.save_file, "error_img_name.txt"), "w") as f:
        for error_img_name in error_img_names:
            f.write(error_img_name + "\n")
